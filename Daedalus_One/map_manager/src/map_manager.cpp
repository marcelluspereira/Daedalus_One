...
namespace daedalus::map_manager {
	...
	bool MapManager::initialize(sf::RenderWindow* window, const fs::path& basePath)
	{
		m_window = window;
		m_basePath = basePath;

		//Initialize maps
		m_map = parseMap("map1.json");
		...
		return (m_map != nullptr && projectOk);
	}

	std::unique_ptr<tson::Map> MapManager::parseMap(const std::string& filename, std::unique_ptr<tson::IDecompressor<std::vector<uint8_t>, std::vector<uint8_t>>> decompressor)
	{
		tson::Tileson t;
	...
	bool MapManager::parseProject(const std::string& filename)
	{
	...
	void MapManager::updateAnimations()
	{
	...
	void MapManager::drawTexture(const sf::Texture& texture, const sf::IntRect& textureRegion, const sf::Vector2f& position,
		int32_t tileHeight, bool hortFlip, bool vertFlip, bool diagonalFlip)
	{
		...
		sf::Vector2f halfSize = sf::Vector2f(textureRegion.size.x / 2.0f, textureRegion.size.y / 2.0f);
		sprite.setOrigin(halfSize);
		sf::Vector2f originOffset{ halfSize.x, halfSize.y + tileHeight - textureRegion.size.y };
	...
	void MapManager::drawTileLayer(tson::Layer& layer)//, tson::Tileset* tileset)
	{
	...
	void MapManager::drawImageLayer(tson::Layer& layer)
	{
	...
	void MapManager::drawObjectLayer(tson::Layer& layer)
	{
		...
		for (auto& obj : layer.getObjects())
		{
			...
			case tson::ObjectType::Object:
			{
				...
				else if (tileset->getType() == tson::TilesetType::ImageTileset)
				{
					...
					textureRegion = { {static_cast<int32_t>(offset.x), static_cast<int32_t>(offset.y)},
				...
				if (texture != nullptr)
				{
					...
					sprite.setRotation(sf::degrees(originalRotation)); //Since we used a shared sprite for this example, we must reset the rotation.
				}
			}
			break;
	 ...
	 * @return sf::Texture
	 */
	const sf::Texture* MapManager::getTexture(const std::string& image)
	{
	 ...
	 * @return
	 */
	sf::Sprite* MapManager::storeAndLoadImage(const std::string& image, const sf::Vector2f& position)
	{
	...
	sf::Vector2f MapManager::getTileOffset(int tileId, tson::Map* map, tson::Tileset* tileset)
	{
	...
	void MapManager::drawLayer(tson::Layer& layer)
	{
	 ...
	 */
	template <typename T>
	fs::path MapManager::getImagePath(const T& tileset)
	{
		//fs::path path = fs::path(fs::path("../") / tileset.getImage().filename());
		fs::path path = fs::path(tileset.getImage().filename());
		return path;
	}
...
#if __clang__
	fs::path MapManager::getMacApplicationFolder(bool isAppPath)
	{
		char buf[PATH_MAX];
